...
- The `2026-04-30-101438.pcap` add-on showed `xs.gdt.qq.com` during the swipe-up Taobao ad; it is now mapped to a tiny local close/back page instead of a hard REJECT to avoid leaving a blank network-error WebView.
- The `2026-04-30-104745.har` blank-page capture exposed cached GDT splash media on `adsmind.gdtimg.com`; only the verified `ads_svp_video__*.mp4` material path is suppressed.
