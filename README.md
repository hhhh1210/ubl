...
- `Map Local` adds exact material fallbacks for the observed Taobao material video/image, Baidu splash SDK backgrounds/preload probe, Baidu win/loss tracking pixel, GDT material, and Pangle signed image hosts. App config, AB test, reader config, account, bookstore, encrypted `wtzw.com` config, QttUnion, AICLK, and generic SDK launch/config endpoints are intentionally allowed until a HAR shows safe field-level cleanup.
