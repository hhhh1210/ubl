...
- The 2026-05-08 HAR identifies the next one-shot popup as `p_home_other_banner` / `kf_home_other_title_image` opening `kf-webx/pop-ups/upgrade-fission.html`; the module now cleans the narrow `api.didi.cn/webx/na/product/init` WebX NA source and returns 204 for the verified `upgrade-fission` document so the native WebX popup host does not keep an empty gray overlay.
